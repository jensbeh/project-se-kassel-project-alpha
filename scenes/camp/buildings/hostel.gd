...
func _ready():
	...
	if Utils.get_music_player().stream != Constants.PreloadedMusic.Hostel:
		Utils.set_and_play_music(Constants.PreloadedMusic.Hostel)
...
func interaction_detected():
	if Utils.get_current_player().is_in_change_scene_area():
		...
		print("HOSTEL: Change scene to \""  + str(next_scene_path) + "\"")
...
func body_exited_change_scene_area(body, changeSceneArea):
	if body.name == "Player":
		print("HOSTEL: Body \""  + str(body.name) + "\" EXITED changeSceneArea \"" + changeSceneArea.name + "\"")
		current_area = null
